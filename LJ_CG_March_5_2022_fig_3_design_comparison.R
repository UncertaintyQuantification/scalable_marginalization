...
##LJ 
##plot(seq(0.8,5,0.1),LJ_kernel(seq(0.8,5,0.1)))

##############three design 

S=3
record_rmse_CG=matrix(NA,S)
N=200
testing_n =100
...
for(i_S in 1:S){
   ...
   # record_time_CG[i_S,j_repeat]=system.time(
    ...
    record_phi_pred[,i_S]=pred_mean_CG_LJ_multi_step
    #record_phi_truth[,i_S]=testing_output
}


###compute the uncertainty for the last one
c_star=rep(NA,testing_n)
system.time(
  for(i in 1:testing_n ){
    print(i)
    Ur=sparse_A_times_x( A,   P=permutation_A, r[i,])
...
pdf('estimation_truncated_LJ_n_200.pdf',height=4,width=4)
plot(testing_d,testing_output,type='l',col='black',lty=1,xlab='d',ylab=expression(phi),mgp=c(2.5,1,0))

polygon( c(testing_d,rev(testing_d)), c(LB95,rev(UB95)), col = "grey80", border = F)
lines(testing_d,testing_output,type='l',col='black',lty=1)
lines(testing_d,record_phi_pred[,1],type='l',col='green',lty=2)

lines(testing_d,record_phi_pred[,2],type='l',col='orange',lty=2)

lines(testing_d,record_phi_pred[,3],type='l',col='blue',lty=2)


legend("bottomright",lty=c(2,2,2,1),
       col=c('green','orange','blue','black'),
       legend=c( 'CG GP, uniform','CG GP, normal','CG GP, log-uniform','Truth')
)

dev.off()

sqrt(mean( (record_phi_pred[,1]-testing_output)^2))
sqrt(mean( (record_phi_pred[,2]-testing_output)^2))
sqrt(mean( (record_phi_pred[,3]-testing_output)^2))
